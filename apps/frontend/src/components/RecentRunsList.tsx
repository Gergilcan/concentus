...
export function RecentRunsList({
  ...
  if (runs.length === 0) return <div className={styles.sideEmpty}>{t('Nothing has run yet.')}</div>
      ...
      {runs.map((r) => (
        <li key={r.id} className={styles.runItem}>
          <button className={styles.runRow} onClick={() => onOpenRun(r.id)}>
            ...
            <span className={styles.runFlow}>{r.flowName || t('flow')}</span>
          ...
          <button className={styles.retry} title={t('Re-run with the same input')} onClick={() => onRetryRun(r.id)}>
            ⟳
          </button>
        </li>
      ))}
