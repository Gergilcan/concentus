...
describe('permissions', () => {
  ...
  it('lets an operator run without letting it edit', () => {
    const p = permissionsFor('OPERATOR')
    expect(p).toMatchObject({ canRun: true, canEdit: false, canAdminister: false })
  })

  it('lets a member edit, and an admin administer', () => {
    expect(permissionsFor('MEMBER')).toMatchObject({ canRun: true, canEdit: true, canAdminister: false })
    expect(permissionsFor('ADMIN')).toMatchObject({ canRun: true, canEdit: true, canAdminister: true })
  })

  // There is no longer a mode without accounts, so no role means the session has not resolved
  // yet. A screen that briefly offers to delete things while it works that out is worse than one
  // that briefly offers too little.
  it('gives an unresolved session nothing', () => {
    expect(permissionsFor(null)).toMatchObject({ canRun: false, canEdit: false, canAdminister: false })
  })

  it('treats an unknown role as the least privileged, rather than as an admin', () => {
    expect(permissionsFor('editor')).toMatchObject({ canRun: false, canEdit: false })
    expect(permissionsFor(undefined).canRun).toBe(false)
  })

  it('is not case-sensitive about a role that came back from the backend', () => {
    expect(permissionsFor('operator').canRun).toBe(true)
  })
  ...
  it('says which role is refusing and who can change it', () => {
    const reason = deniedReason(permissionsFor('VIEWER'), 'run')
