...
import { ANNUAL_DISCOUNT, MAX_TEAM_SEATS, parseSeats, parseTerm, quote, readPricing } from './pricing.mjs'
...
test('no price in the environment: null, and the page will say "to be announced"', () => {
  const p = readPricing({})
  assert.equal(p.monthlyPerSeat, null)
...
test('quote: annual is twelve months less 10%, rounded once per seat', () => {
  const q = quote({ seats: 3, term: 'annual' }, readPricing({ TEAM_PRICE_MONTHLY_PER_SEAT: '12' }))
  assert.equal(q.unitAmountCents, 12960) // 1200 × 12 × 0.9
  assert.equal(q.totalCents, 38880)
  assert.equal(q.months, 12)
  // A price with cents: 9.99 × 12 × 0.9 = 107.892 → 10789 cents, and the total is an exact multiple of it.
  const odd = quote({ seats: 7, term: 'annual' }, readPricing({ TEAM_PRICE_MONTHLY_PER_SEAT: '9.99' }))
  assert.equal(odd.unitAmountCents, 10789)
  assert.equal(odd.totalCents, 10789 * 7)
})

test('quote without a configured price throws rather than charging zero', () => {
  assert.throws(() => quote({ seats: 1, term: 'monthly' }, readPricing({})), /no price/)
})
