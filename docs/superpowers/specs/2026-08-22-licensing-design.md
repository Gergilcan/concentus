...
| Terms | Monthly or annual (annual −20 %: 25 €/seat/month monthly, 20 €/seat/month paid yearly — decided 29 Aug), never perpetual. Same 14-day grace as enterprise after `expires`. |
| Price | 25 € per seat per month, the default in `pricing.mjs` (`DEFAULT_MONTHLY_PER_SEAT`); `TEAM_PRICE_MONTHLY_PER_SEAT` in Vercel overrides it and `off` unannounces it (the card prints "Pricing to be announced — write in", checkout answers 503). Read by `GET /api/pricing`, never written in HTML. |
